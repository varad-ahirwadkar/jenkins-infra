    ...
    script {
        ...
        try {
            sh '''
                ...
                sed -i "s|upgrade_image:.*$|upgrade_image: ${UPGRADE_IMAGE}|g" upgrade_vars.yaml
                sed -i "s|delay_time:.*$|delay_time: 900|g" upgrade_vars.yaml
                cat upgrade_vars.yaml
                cp ${WORKSPACE}/ocp4_playbooks/examples/inventory .
                sed -i "s|B1.B1.B1.B1|${BASTION_IP}|g" inventory
                ...
                cat inventory
                grep -qxF "[ssh_connection]" ansible.cfg || echo "[ssh_connection]" >> ansible.cfg
                grep -qxF "ssh_args = -C -o ControlMaster=auto -o ControlPersist=120m -o ServerAliveInterval=30" ansible.cfg || echo "ssh_args = -C -o ControlMaster=auto -o ControlPersist=120m -o ServerAliveInterval=30" >> ansible.cfg
                cat ansible.cfg
